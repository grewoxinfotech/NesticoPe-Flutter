...
//         final validItems =
//             controller.items.where((lead) {
//               final custom = lead.customFields;
//               return custom != null && (custom.serviceId?.isNotEmpty ?? false);
//             }).toList();
//
//         log(
//           "My Contractor Data ${controller.items.map((element) => element.toJson())}",
//         );
//         if (validItems.isEmpty) {
//           return const Center(
//             child: Text("No valid contractor services found"),
//           );
//         }
//
...
//
//     try {
//       final result = await widget.controller.checkReviewDone(
//         custom!.serviceId!,
//       );
//       setState(() {
//         hasReviewed = result;
...
//             'Status:',
//             lead.status?.capitalizeFirst ?? 'N/A',
//             Colors.black,
//           ),
//           const SizedBox(height: 8),
//           _buildInfoRow(
    ...
    return Scaffold(
      ...
      body: Obx(() {
        if (controller.isLoading.value) {
          ...
          return const Center(child: Text("No contractor projects found"));
        }

        final validItems =
            controller.items.where((project) {
        ...
        return RefreshIndicator(
          ...
          child: ListView.builder(
            ...
            itemCount: validItems.length,
            itemBuilder: (context, index) {
              final project = validItems[index];
...
class _ContractorCardWidgetState extends State<ContractorCardWidget> {
  ...
  Future<void> _checkReviewStatus() async {
    final meta = widget.project.meta;
    if (meta?.serviceId == null) return;

    setState(() => isLoadingReview = true);

    try {
      final result = await widget.controller.checkReviewDone(meta!.serviceId!);
