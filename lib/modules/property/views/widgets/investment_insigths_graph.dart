...
//       }
//
//       // Collect prices from all datasets to find the one with largest range
//       List<List<num>> allDatasets = [
//         myPropertyData.map((e) => e.price).toList(),
//       ];
//
//       if (_comparisonType == 'locality') {
//         final localityData = _getLocalityChartData();
//         if (localityData.isNotEmpty) {
//           allDatasets.add(localityData.map((e) => e.price).toList());
//         }
//       } else if (_comparisonType == 'property') {
//         final propertyTypeData = _getPropertyTypeChartData();
//         if (propertyTypeData.isNotEmpty) {
//           allDatasets.add(propertyTypeData.map((e) => e.price).toList());
//         }
//
//         for (var prop in _selectedProperties) {
//           if (prop.chartData.isNotEmpty) {
//             allDatasets.add(prop.chartData.map((e) => e.price).toList());
//           }
//         }
//       }
//
//       // Find dataset with largest range and use it for Y-axis calculation
//       List<num> allPrices = [];
//       num largestRange = 0;
//
//       for (var dataset in allDatasets) {
//         if (dataset.isEmpty) continue;
//         final dataMin = dataset.reduce(min);
//         final dataMax = dataset.reduce(max);
//         final dataRange = dataMax - dataMin;
//
//         if (dataRange > largestRange) {
//           largestRange = dataRange;
//           allPrices = dataset;
//         }
//       }
//
...
//               .toDouble();
//
//       return _buildChartUI(context, myPropertyData, minY, maxY, yInterval);
//     });
//   }
//
//   Widget _buildChartUI(
...
class _InvestmentInsightChartState extends State<InvestmentInsightChart> {
  ...
  Widget build(BuildContext context) {
    ...
    return Obx(() {
      ...
      // 2. Collect ALL points from ALL active datasets for Axis Calculation
      List<_ChartPoint> allActivePoints = [...myPropertyData];

      if (_comparisonType == 'locality') {
        final localityData = _getLocalityChartData();
        if (localityData.isNotEmpty) {
          allActivePoints.addAll(localityData);
        }
      } else if (_comparisonType == 'property') {
        final propertyTypeData = _getPropertyTypeChartData();
        if (propertyTypeData.isNotEmpty) {
          allActivePoints.addAll(propertyTypeData);
        }
        for (var prop in _selectedProperties) {
          if (prop.chartData.isNotEmpty) {
            allActivePoints.addAll(prop.chartData);
          }
        }
      }

      // 3. Calculate Global Min/Max for Axis
      if (allActivePoints.isEmpty) {
        return const Center(child: Text('No data available'));
      }

      // X-Axis (Years)
      double minX = allActivePoints.map((e) => e.year).reduce(min).toDouble();
      double maxX = allActivePoints.map((e) => e.year).reduce(max).toDouble();

      // Y-Axis (Prices)
      double minPrice =
          allActivePoints.map((e) => e.price).reduce(min).toDouble();
      double maxPrice =
          allActivePoints.map((e) => e.price).reduce(max).toDouble();
      double range = maxPrice - minPrice;

      if (range <= 0) range = 100000; // Prevent 0 range

      final yInterval = _calculateOptimalInterval(range);
      final minY =
          ((minPrice - range * 0.05) ~/ yInterval * yInterval).toDouble();
      final maxY =
          ((maxPrice + range * 0.05) ~/ yInterval * yInterval + yInterval)
          ...
          Container(
            ...
            child: LineChart(
              LineChartData(
                ...
                borderData: FlBorderData(show: false),
                titlesData: _buildTitles(minX, maxX),
  ...
  LineChartBarData _buildMainPropertyLine(List<_ChartPoint> data) {
    return LineChartBarData(
      ...
      // ),
      dotData: const FlDotData(show: false),
      // CHANGE: Use actual Year as X value, not index
      spots: [
        for (final point in data)
          FlSpot(point.year.toDouble(), point.price.toDouble()),
      ],
    );
  }
  ...
  FlTitlesData _buildTitles(double minX, double maxX) {
    return FlTitlesData(
      show: true,
      leftTitles: AxisTitles(
        sideTitles: SideTitles(
      ...
      bottomTitles: AxisTitles(
        sideTitles: SideTitles(
          ...
          reservedSize: 28,
          getTitlesWidget: (value, meta) {
            // value is the Year (e.g., 2020.0)
            final int year = value.toInt();

            // Only show labels for years within range
            if (value < minX || value > maxX) return const SizedBox.shrink();

            // Logic to skip labels if crowded (e.g., show every 2nd year if even)
            // Or just show all if range is small.
            // Using logic: if year is even, show it.
            if (year % 2 != 0) return const SizedBox.shrink();

            final currentYear = DateTime.now().year;
            final isPast = year <= currentYear;
            ...
            return Padding(
              ...
              child: Text(
                ...
                style: TextStyle(
                  fontSize: 10,
                  fontWeight: FontWeight.w500,
                  color: isPast ? Colors.grey[700] : Colors.grey[500],
                ),
              ),
            );
          },
