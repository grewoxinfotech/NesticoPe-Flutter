...
class ContractorMyService {
  ...
  Future<void> changeActiveToInActive(String id, bool isActive) async {
    ...
    try {
      final response = await http.put(uri, headers: await headers(),body: jsonEncode({
        "isActive": isActive
      }));
  ...
  Future<bool> createService(Map<String, dynamic> data) async {
    ...
    try {
      final response = await http.post(uri, headers: await headers(),body: jsonEncode({'data':data}));
